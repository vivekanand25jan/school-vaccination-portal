...
class Students extends CI_Controller {
	...
	function addRegisterPage(){
		...
		$data['rec'] = $this->student_model->getAllDriveForStudent($studentId);
		$data['error'] = (isset($data['rec']) && $data['rec']!=false)?false:true;
		$this->load->view('ajax/vaccine_register',$data);
	}

	public function getVaccineDoneList(){
		$studentId = $this->uri->segment(3);
		$data['rec'] = $this->student_model->getVaccineDoneList($studentId);
		$data['error'] = (isset($data['rec']) && $data['rec']!=false)?false:true;
		$this->load->view('ajax/vaccinated_student_list',$data);
	}

	public function registerForVaccination(){

		$dataResult = $this->student_model->registerForVaccination($_POST);
		$data['error'] = ($dataResult==true)?'pass':'fail';
		$this->load->view('ajax/vaccine_register_status',$data);
	}
...
/*  STUDENTS */
	public function getAllStudents(){
		$dataArr = $this->student_model->getAllStudent(); 
		//$dataArr = $this->getAlltableData('student','','',array());
	...
	public function addStudentAction(){
		...
		$this->form_validation->set_rules('roll', 'roll', 'required|is_unique[student.student_roll]');
		$this->form_validation->set_rules('useremail', 'Email', 'trim|required|valid_email|is_unique[student.student_email]');
		...
		else
		{
			...
			$data = array('student_first_name'=>$this->input->post('FirstName'),
						'student_last_name'=>$this->input->post('LastName'),
						'class'=>$this->input->post('studentClass'),
						'student_roll'=>$this->input->post('roll'),
						'student_email'=>$this->input->post('useremail'),
						'student_gender'=>$this->input->post('userGender'),
						  'dateCreated'=>date("Y-m-d h:i:s")
						  ); 
			$this->insertInTable('student',$data);
			redirect('students/getAllStudents');
		}
	}

	public function editStudent(){
		$agencyId = $this->uri->segment(3);
		$opt = array('studentId' => $agencyId);
		$this->getDataFromTableById('student','student','edit-student',$opt);
	}
	...
	public function editStudentAction(){
		...
		{ 
			...
			$this->changeStatusTable('student',$data,array('studentId'=>$agencyid));
			redirect('students/getAllStudents');
		}
	...
	public function changeStudentStatus(){
		...
		$dataArr = $this->changeStatusTable('student',array('status'=>$statusId),array('studentId'=>$agencyId));
		redirect('students/getAllStudents');
	}
	...
	public function deleteStudent(){
		$agencyId = $this->uri->segment(3); 
		$dataArr = $this->deleteResultFromTable('student',array('studentId'=>$agencyId));
		redirect('students/getAllStudents');
	}
			...
			if($redirectClass!=""){
				if($tableName!=""){
					...
					if($dataArr!=FALSE){
						...
						redirect('students/'.$redirectClass);
					}
				}else{
					redirect('students/'.$redirectClass);
				}

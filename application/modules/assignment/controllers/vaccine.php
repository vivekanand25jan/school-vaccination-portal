...
/*  STUDENTS */
	...
	public function addVaccineRecord(){
		$this->load->view('add-vaccine');
	}


	public function addVaccineAction(){ 
		$this->form_validation->set_rules('vaccine_name', 'Vaccine Name', 'required');
		$this->form_validation->set_rules('vaccine_total_count', 'Vaccine Count', 'required');
		...
		else
		{
			...
			$data = array('vaccine_name'=>$this->input->post('vaccine_name'),
						'totalCount'=>$this->input->post('vaccine_total_count'),
						'createdDate'=>date("Y-m-d h:i:s")
						  );
			$this->insertInTable('vaccine',$data);
			redirect('vaccine/getAllVaccine');
		}
	}

	public function editVaccine(){
		$agencyId = $this->uri->segment(3);
		$opt = array('vaccineId' => $agencyId);
		$this->getDataFromTableById('vaccine','vaccine','edit-vaccine',$opt);
	}

	public function editVaccineAction(){
		$agencyid = $this->input->post('agencyId');
		$this->form_validation->set_rules('vaccine_name', 'Vaccine Name', 'required');
		$this->form_validation->set_rules('totalCount', 'Vaccine Count', 'required');
		if ($this->form_validation->run() == FALSE)
		{ 
			redirect('vaccine/getAllVaccine');
		}
		...
		{ 
			...
			$this->changeStatusTable('vaccine',$data,array('vaccineId'=>$agencyid));
			redirect('vaccine/getAllVaccine');
		}
		
	}

	public function changeVaccineStatus(){
		$agencyId = $this->uri->segment(3);
		$statusId = ($this->uri->segment(4)==1)?0:1; 
		$dataArr = $this->changeStatusTable('vaccine',array('status'=>$statusId),array('vaccineId'=>$agencyId));
		redirect('vaccine/getAllVaccine');
	}
	...
	public function deleteVaccine(){
		$agencyId = $this->uri->segment(3); 
		$dataArr = $this->deleteResultFromTable('vaccine',array('vaccineId'=>$agencyId));
		redirect('vaccine/getAllVaccine');
	}

...
/*  STUDENTS */
	public function getAllDrive(){
		$dataArr = $this->student_model->getAllDriveList();
		//$dataArr = $this->getAlltableData('drive','','',array());
		$data['defaultData'] = $dataArr;
		$this->load->view('drive',$data);
	}

	public function addDriveRecord(){
		$dataArr = $this->getAlltableData('drive','','',array());
		$data['vacc'] = $this->getAlltableData('vaccine','','',array());
		$data['defaultData'] = $dataArr;
		$this->load->view('add-drive',$data);
	}


	public function addDriveAction(){ 
		$this->form_validation->set_rules('drive_name', 'Drive Name', 'required');
		$this->form_validation->set_rules('vaccine_name', 'Vaccine Name', 'required');
		$this->form_validation->set_rules('drive_start_date', 'Drive Start Date', 'required');
		$this->form_validation->set_rules('drive_for_class', 'Drive for Class', 'required');
		if ($this->form_validation->run() == FALSE)
		{
			$this->load->view('add-drive');
		}
		else
		{
			...
			$ValidateOptions = array('drive_name'=>$this->input->post('drive_name'),
									'vaccineId'=>$this->input->post('vaccine_name'),
									'drive_start_date'=>$this->input->post('drive_start_date'),
									'drive_for_class'=>$this->input->post('drive_for_class')
				); 
			$VOPtion = $this->verifyTableWithOption('drive' , $ValidateOptions);
			if($VOPtion == false){
			$data = array('drive_name'=>$this->input->post('drive_name'),
						'vaccineId'=>$this->input->post('vaccine_name'),
						'drive_start_date'=>date(("d-m-Y"),strtotime($this->input->post('drive_start_date'))),
						'drive_for_class'=>$this->input->post('drive_for_class'),
						'createdDate'=>date("Y-m-d h:i:s")
						  );
				$this->insertInTable('drive',$data);
				redirect('drive/getAllDrive');
			}else{
				$data['errror_msg'] = "This date is already aloted for other vaccination drive.";
				$this->load->view('add-drive',$data);
			}
		}
	}
	...
	public function editDrive(){
		...
		$dataArr = $this->getAlltableData('vaccine','','',array());
		$data['record'] = $dataArr; 
		$opt = array('driveId' => $agencyId);
		$this->getDataFromTableById('drive','drive','edit-drive',$opt,$data);
	}

	public function editDriveAction(){
		$this->form_validation->set_rules('drive_name', 'Drive Name', 'required');
		$this->form_validation->set_rules('vaccine_name', 'Vaccine Name', 'required');
		$this->form_validation->set_rules('drive_start_date', 'Drive Start Date', 'required');
		$this->form_validation->set_rules('drive_for_class', 'Drive for Class', 'required');
		...
		{ 
			...
			$this->changeStatusTable('drive',$data,array('driveId'=>$this->input->post('agencyId')));
			redirect('drive/getAllDrive');
		}
		
	}

	public function changeDriveStatus(){
		$agencyId = $this->uri->segment(3);
		$statusId = ($this->uri->segment(4)==1)?0:1; 
		$dataArr = $this->changeStatusTable('drive',array('status'=>$statusId),array('driveId'=>$agencyId));
		redirect('drive/getAllDrive');
	}
	...
	public function deleteDrive(){
		$agencyId = $this->uri->segment(3); 
		$dataArr = $this->deleteResultFromTable('drive',array('driveId'=>$agencyId));
		redirect('drive/getAllDrive');
	}
...
/* END STUDENTS */
	...
	/* GLOBAL FUNCTION */
		...
		public function insertInTable($tableName,$option){
			...
			$dataArr = $this->student_model->insertInTable($tableName,$option); 
		...
		public function verifyTableWithOption($tableName , $options){
			$dataArr = $this->student_model->checkTableWithOption($tableName,$options); 
			return $dataArr;
		}
		...
		public function getAlltableData($tableName,$redirectClass='',$succesPage='',$option){
			if($redirectClass!=""){
				if($tableName!=""){
					$dataArr = $this->student_model->getAlltableData($tableName,$option); 
					if($dataArr!=FALSE){
						$data['defaultData'] = $dataArr; 
						$this->load->view($succesPage,$data);
					}else{
						redirect('drive/'.$redirectClass);
					}
				}else{
					redirect('drive/'.$redirectClass);
				}
